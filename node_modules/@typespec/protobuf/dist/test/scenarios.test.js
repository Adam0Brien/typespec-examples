...
const TypeSpecProtobufTestLibrary = {
            ...
            it(scenarioName, async function () {
                ...
                if (shouldRecord) {
                    ...
                    if (!statSync(expectationDirectory, { throwIfNoEntry: false })) {
                        assert.strictEqual(Object.entries(emitResult.files).length, 0, "no expectations exist, but output files were generated");
                    }
                    else {
 ...
 */
function removeCoreDiagnostics(diagnostics) {
...
async function doEmit(files, options) {
    ...
    const [, diagnostics] = await host.compileAndDiagnose("main.tsp", {
        ...
        noEmit: false,
        emitters: {
            "@typespec/protobuf": options,
        },
    });
    return {
        files: Object.fromEntries([...host.fs.entries()]
...
function assertFilesAsExpected(outputFiles, expectedFiles) {
    for (const fn of Object.keys(expectedFiles)) {
        assert.ok(Object.prototype.hasOwnProperty.call(outputFiles, fn), `expected file ${fn} was not produced`);
    }
 ...
 */
async function writeExpectationDirectory(expectationDirectory, outputFiles) {
...
async function readdirRecursive(dir, base = dir) {
    ...
    for (const entry of (await readdir(dir)).map((e) => path.join(dir, e))) {
        ...
        if (stats.isDirectory()) {
            ...
            const correctedPath = process.platform === "win32" ? relativePath.replace(/\\/g, "/") : relativePath;
